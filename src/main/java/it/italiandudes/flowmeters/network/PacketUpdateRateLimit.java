    ...
    public IMessage onMessage(PacketUpdateRateLimit message, MessageContext ctx) {
      ...
      world.addScheduledTask(() -> {
        if (!world.isBlockLoaded(pos)) {
          FlowMeters.LOGGER.error("Received PacketUpdateRateLimit for unloaded position {}", pos);
          return;
        }
        ...
        if (tile instanceof TileEntityEnergyMeterBase) {
          ...
          FlowMeters.LOGGER.info("Received PacketUpdateRateLimit for {}", pos);
        } else if (tile instanceof TileEntityFluidMeterBase) {
          ((TileEntityFluidMeterBase) tile).handleRateLimitChangeRequest(message.rateLimit);
          FlowMeters.LOGGER.info("Received PacketUpdateRateLimit for {}", pos);
        } else {
          FlowMeters.LOGGER.error(
              "Received PacketUpdateRateLimit for position with no TE: {}", pos);
        }
      });
