...
import it.italiandudes.flowmeters.client.model.FluidMeterTextureLocations;
import it.italiandudes.flowmeters.network.PacketUpdateFluidMeterConfig;
import it.italiandudes.flowmeters.network.PacketUpdateMeterSides;
import it.italiandudes.flowmeters.network.PacketUpdateRateLimit;
...
public class GuiFluidMeter extends GuiScreen {
  private static final ResourceLocation BACKGROUND_TEXTURE = new ResourceLocation(
      FlowMeters.MODID, "textures/gui/fluidmeter.png");
  ...
  private final TileEntityFluidMeterBase tile;
  ...
  private final ResourceLocation sideTexture = FluidMeterTextureLocations.getGuiResource(FluidMeterTextureLocations.SIDE);
  private final ResourceLocation inputTexture = FluidMeterTextureLocations.getGuiResource(FluidMeterTextureLocations.INPUT);
  private final ResourceLocation outputTexture = FluidMeterTextureLocations.getGuiResource(FluidMeterTextureLocations.OUTPUT);
  private final ResourceLocation screenTexture = FluidMeterTextureLocations.getGuiResource(FluidMeterTextureLocations.SCREEN);

  private GuiTextField rateLimitTextField;
  private boolean isEditingLimit = false;


  public GuiFluidMeter(TileEntityFluidMeterBase tile) {
    this.tile = tile;
  ...
  private String getRateLimitString() {
    return this.tile.getRateLimit() == TileEntityFluidMeterBase.UNLIMITED_RATE
        ? "Unlimited"
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    int titleWidth = this.fontRenderer.getStringWidth("Fluid Meter");
    int titleX = (TEXTURE_WIDTH - titleWidth) / 2;
    this.fontRenderer.drawString("Fluid Meter", x + titleX, y + 7, COLOR_GREY);
    ...
    this.fontRenderer.drawString(RATE_FORMAT.format(tile.getTransferRate() / tile.getFluidScale()) + " " + units + "/t", x  + leftPanelOffset, statY + 11, COLOR_WHITE);
    statY += statYIncr;
    ...
    this.fontRenderer.drawString(TOTAL_FORMAT.format(tile.getTotalFluidTransferred() / tile.getFluidScale()) + " " + units, x  + leftPanelOffset, statY + 11, COLOR_WHITE);
    statY += statYIncr;
  ...
  protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
    ...
    if (mouseButton == 0) {
      ...
      if (this.setRateLimitButton.isMouseOver() && this.setRateLimitButton.visible && this.isEditingLimit) {
        String str = this.rateLimitTextField.getText();
        if (Util.isValidRateLimitString(str)) {
          int rateLimit;
          if (str.isEmpty()) {
            rateLimit = TileEntityFluidMeter.UNLIMITED_RATE;
          } else {
          ...
          FlowMeters.NETWORK.sendToServer(new PacketUpdateRateLimit(this.tile.getPos(), rateLimit));
        }
      }

      if (this.rateLimitButton.isMouseOver() && !this.isEditingLimit) {
        this.isEditingLimit = true;
        this.rateLimitTextField.setText(
            this.tile.getRateLimit() == TileEntityFluidMeterBase.UNLIMITED_RATE
                ? ""
                : Integer.toString(this.tile.getRateLimit()));
      }
  ...
  private void sideConfigButtonClicked(GuiButtonSideConfig button) {
    ...
    FlowMeters.NETWORK.sendToServer(new PacketUpdateMeterSides(
        this.tile.getPos(),
  ...
  protected void actionPerformed(@NotNull GuiButton button) throws IOException {
    ...
    if (sendUpdatePacket) {
      FlowMeters.NETWORK.sendToServer(
          new PacketUpdateFluidMeterConfig(
            this.tile.getPos(),
